...
```
По умолчанию просто возвращает JSON со всеми задачами. Чтобы одновременно сохранить файл на сервере, добавьте параметр `saveToDisk=true`:
```
GET /api/tasks/export?saveToDisk=true
```
Файл будет записан по пути, указанному в `ExportService:ExportFilePath` (по умолчанию `tasks_export.json` в корне проекта).
